...
//        Log.selectLog(logLevel: .debug, "deviceHeight:\(deviceHeight)")
    ...
    class func getDeviceInfo() -> String {
        ...
        Log.selectLog(logLevel: .debug, "machine:\(machine)")
        ...
        Log.selectLog(logLevel: .debug, "screenWidth:\(screenWidth)")
        Log.selectLog(logLevel: .debug, "screenHeight:\(screenHeight)")
        
        if let deviceName = deviceCodeDic[code] {
            Log.selectLog(logLevel: .debug, "deviceName:\(deviceName)")
            
            if deviceName == "Simulator" {
                if screenWidth == 640.0 && screenHeight == 1136.0 {
                    return "iPhoneSE"
                } else {
                    return "else iPhone"
                }
