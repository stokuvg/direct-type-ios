...
private extension InitialInputRegistVC {
    ...
    func trySignUp() {
        ...
        AWSMobileClient.default().signUp(username: phoneNumber, password: Constants.AuthDummyPassword, userAttributes: phoneNumberAttribute) { (signUpResult, error) in
            if let error = error {
    ...
    func transitionToComfirm() {
        ...
        vc.configure(with: InitialInputConfirmVC.LoginInfo(phoneNumberText: phoneNumberText, password: Constants.AuthDummyPassword), delegate: self)
